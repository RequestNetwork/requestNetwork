...
export default class EtherscanProvider
  ...
  public async getGasPrice(
    ...
    if (apiResponse.status && apiResponse.status !== '1') {
      throw new Error(
        `Etherscan error: ${apiResponse.message} ${apiResponse.result}`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Etherscan API response doesn't contain the correct format`
      );
    }
    ...
    if (!EthereumUtils.isGasPriceSafe(apiGasPrice)) {
      throw Error(
        `Etherscan provided gas price not safe to use: ${apiGasPrice}`
      );
    }
