...
const btcProviderMock0: Types.IBitcoinProvider = {
  getAddressInfo: async (): Promise<Types.IBalanceWithEvents> => ({ balance: '0', events: [] }),
  parse: (): Types.IBalanceWithEvents => ({ balance: '0', events: [] }),
};

const btcProviderMock1: Types.IBitcoinProvider = {
  getAddressInfo: async (): Promise<Types.IBalanceWithEvents> => ({ balance: '1', events: [] }),
  parse: (): Types.IBalanceWithEvents => ({ balance: '1', events: [] }),
};

const btcProviderMock2: Types.IBitcoinProvider = {
  getAddressInfo: async (): Promise<Types.IBalanceWithEvents> => ({ balance: '2', events: [] }),
  parse: (): Types.IBalanceWithEvents => ({ balance: '2', events: [] }),
};

const btcProviderMockMinus1: Types.IBitcoinProvider = {
  getAddressInfo: async (): Promise<Types.IBalanceWithEvents> => ({ balance: '-1', events: [] }),
  parse: (): Types.IBalanceWithEvents => ({ balance: '-1', events: [] }),
};
...
describe('api/btc/bitcoin-info-retriever', () => {
  describe('getAddressInfo', () => {
    it('should give the right value with two identical response', async () => {
      const btcRetriever = new BitcoinInfoRetriever();
      btcRetriever.providers = [btcProviderMock1, btcProviderMock1];

      await expect(
        btcRetriever.getAddressInfo(0, 'address', Types.EVENTS_NAMES.PAYMENT),
