...
- `Types.Payment.PAYMENT_NETWORK_ID.ETH_PROXY_CONTRACT`('pn-eth-proxy-contract'): uses an intermediary contract to document which request is being paid, through the `PaymentReference`. (see [the specification](https://github.com/RequestNetwork/requestNetwork/blob/master/packages/advanced-logic/specs/payment-network-eth-proxy-contract-0.1.0.md))
