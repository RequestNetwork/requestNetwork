...
library LibChainlinkConversion {
   ...
   */
  function getDecimals(address _addr) internal returns (uint256 decimals) {
    // by default we assume it is fiat
    decimals = FIAT_DECIMALS;

    bytes4 getNativeHashSelector = bytes4(keccak256('getNativeTokenHash()'));
    bytes memory getNativeHashCall = abi.encodeWithSelector(getNativeHashSelector);
    (bool success, bytes memory nativeTokenHashData) = address(this).delegatecall(
      getNativeHashCall
    );
    require(success, 'Could not get conversion rate');

    address nativeTokenHash = abi.decode(nativeTokenHashData, (address));
