...
export const getProxyAddress = (
  ...
  const deploymentInfo = getDeploymentInformation(request.currencyInfo.network, pn.version);
  if (!deploymentInfo) {
    throw new Error(
      `No deployment found for network ${request.currencyInfo.network}, version ${pn.version}`,
    );
  }
