...
describe('payEthBatchProxyRequest', () => {
  it('should throw an error if the one request is not eth', async () => {
  ...
  it('should pay an ETH batch of 2 requests', async () => {
    ...
    expect(balanceEthBefore.toString()).toBe(
      balanceEthAfter
        ...
        .add('4') // = 2 * Nb_txs, and Nb_txs = 2
        .add(confirmedTx.gasUsed?.mul(tx?.gasPrice ?? 1))
        .toString(),
    );
    expect(balanceFeeEthAfter.toString()).toBe(balanceFeeEthBefore.add('4').toString()); // = 2 * Nb_txs, and Nb_txs = 2
