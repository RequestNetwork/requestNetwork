...
describe('swap-erc20-fee-proxy', () => {
  ...
  describe('swapErc20FeeProxyRequest', () => {
    ...
    it('should swap and pay with an ERC20 request with fees', async () => {
      ...
      expect(BigNumber.from(balanceAlphaAfter).toNumber()).toEqual(
        BigNumber.from(balanceAlphaBefore).sub(204).toNumber(),
      );
      expect(BigNumber.from(issuerBalanceErc20After).toNumber()).toEqual(
        BigNumber.from(issuerBalanceErc20Before).add(100).toNumber(),
      );
      expect(BigNumber.from(feeBalanceErc20After).toNumber()).toEqual(
        BigNumber.from(feeBalanceErc20Before).add(2).toNumber(),
      );
    });
  });
});
