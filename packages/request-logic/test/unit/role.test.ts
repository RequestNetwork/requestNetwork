...
describe('Role', () => {
    it('can getRole() from object with payee and payer', () => {
        ...
        expect(Role.getRole(obj, payee), 'getRole("") error').to.be.equal(RequestEnum.REQUEST_LOGIC_ROLE.PAYEE);
        expect(Role.getRole(obj, payer), 'getRole("") error').to.be.equal(RequestEnum.REQUEST_LOGIC_ROLE.PAYER);
        expect(Role.getRole(obj, otherId), 'getRole("") error').to.be.equal(RequestEnum.REQUEST_LOGIC_ROLE.THIRD_PARTY);
    });
});
