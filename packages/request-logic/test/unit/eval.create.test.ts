...
describe('requestLogic.applyTransactionToRequestState(Creation)', () => {
    it('can create with only the payee', () => {
        ...
        expect(request.requestId, 'requestId is wrong').to.equal(CryptoUtils.normalizeKeccak256Hash(txCreation.transaction));
        ...
        expect(request.creator.type, 'request.creator.type is wrong').to.equal(RequestEnum.REQUEST_LOGIC_IDENTITY_TYPE.ETHEREUM_ADDRESS);
        ...
        if (request.payee) {
            expect(request.payee.type, 'request.payee.type is wrong').to.equal(RequestEnum.REQUEST_LOGIC_IDENTITY_TYPE.ETHEREUM_ADDRESS);
    ...
    it('can create with only the payer', () => {
        ...
        if (request.payer) {
            expect(request.payer.type, 'request.payer.type is wrong').to.equal(RequestEnum.REQUEST_LOGIC_IDENTITY_TYPE.ETHEREUM_ADDRESS);
        ...
        try {
            const signedTx = {
                signature: {
                    ...
                    value: '0xdd44c2d34cba689921c60043a78e189b4aa35d5940723bf98b9bb9083385de316333204ce3bbeced32afe2ea203b76153d523d924c4dca4a1d9fc466e0160f071c',
                },
                transaction: {
            ...
            expect(e.message, 'exception not right').to.be.equal('transaction.parameters.payee or transaction.parameters.payer must be given');
        }
    });
    ...
    it('cannot create with amount not a number', () => {
        try {
            ...
            expect(e.message, 'exception not right').to.be.equal('transaction.parameters.expectedAmount must be a string representing a positive integer');
        }
    });
    ...
    it('it cannot apply creation with a state', () => {
        try {
            ...
            expect(e.message, 'exception not right').to.be.equal('no request state is expected at the creation');
        }
    });
