...
export function getContractAbi(): any {
  const artifactFilename: string = artifactsEthereumProxy[ARTIFACTS_VERSION].artifact;
...
function getDeploymentInformation(
  ...
  const deploymentInformation = artifactsEthereumProxy[ARTIFACTS_VERSION].deployment[networkName];
