const artifactsProxyChangeCryptoFiat = require('../../artifacts/ProxyChangeCryptoFiat/artifacts.json');
const ARTIFACTS_VERSION: string = artifactsProxyChangeCryptoFiat.lastVersion;
...
export function getContractAbi(): any {
  const artifactFilename: string = artifactsProxyChangeCryptoFiat[ARTIFACTS_VERSION].artifact;

  const artifact = require(`../../artifacts/ProxyChangeCryptoFiat/${artifactFilename.replace(
    /\.[^/.]+$/,
    '',
  )}.json`);
...
export function getDeploymentInformation(
  ...
  const deploymentInformation = artifactsProxyChangeCryptoFiat[artifactsVersion].deployment[networkName];
