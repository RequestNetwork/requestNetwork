const artifactsChainlinkConversionPath = require('../../artifacts/ChainlinkConversionPath/artifacts.json');
const ARTIFACTS_VERSION: string = artifactsChainlinkConversionPath.lastVersion;
...
export function getContractAbi(): any {
  const artifactFilename: string = artifactsChainlinkConversionPath[ARTIFACTS_VERSION].artifact;

  const artifact = require(`../../artifacts/ChainlinkConversionPath/${artifactFilename.replace(
    /\.[^/.]+$/,
    '',
  )}.json`);
...
export function getDeploymentInformation(
  ...
  const deploymentInformation = artifactsChainlinkConversionPath[artifactsVersion].deployment[networkName];
