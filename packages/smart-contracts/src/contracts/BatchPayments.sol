...
contract BatchPayments is Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    address _paymentEthereumFeeProxy,
    address _owner
  ) {
  ...
  function batchERC20PaymentsWithReference(
    ...
    require(
      safeTransferFrom(_tokenAddress, _feeAddress, (batchAmount * batchFee) / 1000),
  ...
  function batchERC20PaymentsMultiTokensWithReference(
    ...
    for (uint256 i = 0; i < uniqueTokens.length && amountByToken[i] > 0; i++) {
      require(
        safeTransferFrom(uniqueTokens[i], _feeAddress, (amountByToken[i] * batchFee) / 1000),
        'batch fee transferFrom() failed'
      );
      amountByToken[i] = 0;
    }
