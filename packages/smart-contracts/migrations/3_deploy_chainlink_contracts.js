const Currency = require('@requestnetwork/currency').Currency;
...
module.exports = async function (deployer) {
  ...
  const ETH_address = new Currency({ type: 'ETH', value: 'ETH' }).getHash();
  const USD_address = new Currency({ type: 'ISO4217', value: 'USD' }).getHash();
  const EUR_address = new Currency({ type: 'ISO4217', value: 'EUR' }).getHash();
  ...
  // erc20SwapConversion
  const erc20SwapConversion = await deployer.deploy(
