...
export default class TransactionIndex implements DataAccessTypes.ITransactionIndex {
   ...
   */
  public async addTransaction(dataId: string, header: DataAccessTypes.IBlockHeader, timestamp: number): Promise<void> {
  ...
  public async getStorageLocationList(
    ...
    // get transaction location for given channel
    let storageLocationList = await this.locationByTopic
    ...
    if (timestampBoundaries) {
      storageLocationList = await Bluebird.filter(storageLocationList, (dataId: string) => this.timestampByLocation.isDataInBoundaries(dataId, timestampBoundaries));
    }
    ...
    if (timestampBoundaries) {
      ...
      for (const channelId of channelIds) {
        ...
        for (const txLocation of txLocations) {
          const isInBoundaries = await this.timestampByLocation.isDataInBoundaries(txLocation, timestampBoundaries);
          if (isInBoundaries) {
