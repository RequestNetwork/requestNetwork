...
/**
 * Implementation of the payment network to pay in Ethereum based on a reference provided to a proxy contract.
 ...
 * Every Ethereum ethereum transaction that reaches these addresses through the proxy contract and has the correct reference will be interpreted as a payment or a refund.
 ...
 */
const ethereumProxyContract: ExtensionTypes.PnReferenceBased.IReferenceBased = {
...
/**
 * Creates the extensionsData to create the extension Ethereum proxy contract payment detection
 *
...
function createCreationAction(
  ...
  return ReferenceBased.createCreationAction(
    ExtensionTypes.ID.PAYMENT_NETWORK_ETH_PROXY_CONTRACT,
    creationParameters,
  );
}
...
function applyActionToExtension(
  ...
  if (
    ...
    throw Error(
      `This extension can be used only on Ethereum requests and on supported networks ${supportedNetworks.join(
        ', ',
      )}`,
    );
  }
...
export default ethereumProxyContract;
