...
contract('RequestEthereum Accept',  function(accounts) {
	...
	it("accept if Core Paused OK", async function () {
		...
		var r = await requestEthereum.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payer});
	...
	it("accept request Ethereum pause impossible", async function () {
		...
		await utils.expectThrow(requestEthereum.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payer}));
	});

	it("accept request not exist impossible", async function () {
		await utils.expectThrow(requestEthereum.acceptAction(666, {from:payer}));
	});

	it("accept request from a random guy impossible", async function () {
		await utils.expectThrow(requestEthereum.acceptAction(utils.getRequestId(requestCore.address, 1), {from:otherguy}));
	});
	it("accept request from payee impossible", async function () {
		await utils.expectThrow(requestEthereum.acceptAction(utils.getRequestId(requestCore.address, 1), {from:payee}));
	});
