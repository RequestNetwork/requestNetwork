...
contract('RequestEthereum Cancel by payer',  function(accounts) {
	...
	it("cancel by payer if Core Paused OK", async function () {
		...
		var r = await requestEthereum.cancelAction(utils.getRequestId(requestCore.address, 1), {from:payer});
	...
	it("cancel by payer request Ethereum pause impossible", async function () {
		...
		await utils.expectThrow(requestEthereum.cancelAction(utils.getRequestId(requestCore.address, 1), {from:payer}));
	});
